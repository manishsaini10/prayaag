...
<div x-data="popupEditor({{ json_encode(isset($popup) ? [
    ...
    'page_targeting_mode' => $popup->targetingRules->isEmpty() ? 'all' : (collect($popup->targetingRules)->first(function($r) { return $r->condition === 'is_not'; }) ? 'except' : 'specific'),
    'page_paths' => $popup->targetingRules->map(function($r) { return $r->value; })->values()->toArray() ?: [],
    ...
    <div class="xl:col-span-2 space-y-6">
        <form method="POST" action="{{ isset($popup) ? url('/admin/popup-builder/' . $popup->id) : url('/admin/popup-builder') }}" class="space-y-6">
                        ...
                        <div class="block-card" :class="{ 'border-primary': dragIdx === idx }" draggable="false">
                            ...
                            <template x-if="block.type === 'image'">
                                <div class="block-body">
                                    <div class="grid grid-cols-3 gap-3">
                                        <div>
                                            ...
                                            <div class="flex gap-2">
                                                <input type="text" x-model="block.src" class="form-input text-sm flex-1" placeholder="https://... or select">
                                                <button @click="window.dispatchEvent(new CustomEvent('open-media-modal', { detail: { activeTab: 'library', onSelect: (m) => { block.src = m.url; block.alt = m.alt || ''; } } }))" type="button" class="btn-sm" style="border:1px solid var(--border)">Select</button>
                                            </div>
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            <label class="text-xs font-medium" style="color:var(--text-muted)">Link URL (Redirect)</label>
                                            <input type="text" x-model="block.link_url" class="form-input text-sm" placeholder="https://... (Optional)">
                                        </div>
                                    </div>
                                    <div class="grid grid-cols-3 gap-3 mt-3">
                                        <div>
                                            <label class="text-xs font-medium" style="color:var(--text-muted)">Width</label>
                                            <select x-model="block.width" class="form-input text-sm">
                                            ...
                                            <template x-if="block.width === 'custom' || (!['auto','25%','50%','75%','100%'].includes(block.width))">
                                                <input type="text" :value="block.width" @input="block.width = $event.target.value" class="form-input text-xs mt-1" placeholder="e.g. 50% or 200px">
                                            </template>
                                        </div>
                                        <div>
                                            <label class="text-xs font-medium" style="color:var(--text-muted)">Max Width</label>
                                            <select x-model="block.max_width" class="form-input text-sm">
                                    ...
                                    <div class="grid grid-cols-3 gap-3 mt-3">
                                        <div>
                                            <label class="text-xs font-medium" style="color:var(--text-muted)">Image Fit</label>
                                            <select x-model="block.fit" class="form-input text-sm">
                                        ...
                                        <div>
                                            <label class="text-xs font-medium" style="color:var(--text-muted)">Horizontal Align</label>
                                            <select x-model="block.align" class="form-input text-sm">
                                                <option value="left">Left</option>
                                                <option value="center">Center</option>
                                                <option value="right">Right</option>
                                        ...
                                        <div>
                                            <label class="text-xs font-medium" style="color:var(--text-muted)">Vertical Align</label>
                                            <select x-model="block.valign" class="form-input text-sm">
            ...
            {{-- Trigger & Scheduling --}}
            <div class="card">
                <h3 class="font-semibold text-sm mb-4" style="color:var(--text)">Trigger & Scheduling</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                        <label class="form-label">Trigger</label>
                        <select x-model="data.settings.trigger" name="settings[trigger]" class="form-input">
                            @foreach(['page_load' => 'On Page Load','time_delay' => 'Time Delay','scroll_percent' => 'Scroll %','exit_intent' => 'Exit Intent','click' => 'On Click'] as $val => $lbl)
                                <option value="{{ $val }}">{{ $lbl }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div x-show="data.settings.trigger === 'time_delay'">
                        <label class="form-label">Delay (seconds)</label>
                        <input type="number" x-model.number="data.settings.delay" name="settings[delay]" class="form-input" min="0" max="120" step="0.5">
                    </div>
                    <div x-show="data.settings.trigger === 'click'">
                        <label class="form-label">CSS Selector</label>
                        <input type="text" x-model="data.settings.clickSelector" name="settings[clickSelector]" class="form-input" placeholder=".btn-open">
                    </div>
                    <div x-show="data.settings.trigger === 'scroll_percent'">
                        <label class="form-label">Scroll %</label>
                        <input type="number" x-model.number="data.settings.scrollPercent" name="settings[scrollPercent]" class="form-input" min="1" max="100">
                    </div>
                </div>
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="starts_at" value="{{ isset($popup) && $popup->starts_at ? $popup->starts_at->format('Y-m-d\TH:i') : '' }}" class="form-input">
                    </div>
                    <div>
                        <label class="form-label">End Date</label>
                        <input type="datetime-local" name="ends_at" value="{{ isset($popup) && $popup->ends_at ? $popup->ends_at->format('Y-m-d\TH:i') : '' }}" class="form-input">
                    </div>
                </div>
            </div>
            ...
            {{-- Page Targeting --}}
            <div class="card">
                <h3 class="font-semibold text-sm mb-4" style="color:var(--text)">Page Targeting</h3>
                <div class="space-y-4">
                    <div>
                        <select x-model="data.page_targeting_mode" class="form-input">
                            <option value="all">Show on all pages</option>
                            <option value="specific">Show only on specific pages</option>
                            <option value="except">Show on all pages except</option>
                        </select>
                    </div>
                    <template x-if="data.page_targeting_mode !== 'all'">
                        <div>
                            <div class="flex gap-2 mb-2">
                                <input type="text" x-model="data.newPath" @keydown.enter.prevent="addPath" class="form-input text-sm flex-1" placeholder="e.g. /about-us or /events/*">
                                <button @click="addPath" type="button" class="btn-sm" style="border:1px solid var(--border);white-space:nowrap">Add Path</button>
                            </div>
                            <template x-if="data.page_paths.length === 0">
                                <div class="text-xs" style="color:var(--text-muted)">No paths added yet. Enter a URL path above.</div>
                            </template>
                            <div class="flex flex-wrap gap-2">
                                <template x-for="(path, idx) in data.page_paths" :key="idx">
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium" style="background:var(--surface-2);border:1px solid var(--border)">
                                        <span x-text="path"></span>
                                        <button @click="removePath(idx)" type="button" style="border:none;background:none;cursor:pointer;color:var(--danger);font-size:14px;line-height:1">&times;</button>
                                    </span>
                                </template>
                            </div>
                            <div class="text-xs mt-2" style="color:var(--text-muted)">
                                <span x-show="data.page_targeting_mode === 'specific'">Popup will appear only on matching pages.</span>
                                <span x-show="data.page_targeting_mode === 'except'">Popup will appear on all pages EXCEPT those listed.</span>
                                Use <code>*</code> as wildcard, e.g. <code>/events/*</code> matches any event page.
                            </div>
                        </div>
                    </template>
                    <input type="hidden" name="page_targeting_mode" x-model="data.page_targeting_mode">
                    <input type="hidden" name="page_paths" :value="JSON.stringify(data.page_paths)">
                </div>
            </div>

            {{-- Design --}}
            <div class="card">
                <h3 class="font-semibold text-sm mb-4" style="color:var(--text)">Design</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                ...
                <div :style="previewView==='desktop'?'width:100%':previewView==='tablet'?'width:768px;margin:0 auto':'width:375px;margin:0 auto'" style="background:white;min-height:400px;transition:all .3s;max-width:100%;overflow-y:auto">
                    <div class="p-6" style="font-family:system-ui,sans-serif">
                        ...
                        <template x-for="block in data.blocks" :key="block._key">
                            ...
                            {{-- Image block preview --}}
                            <div x-show="block.type === 'image'" class="mb-4 flex" :style="{ justifyContent: (block.align === 'left' ? 'flex-start' : (block.align === 'right' ? 'flex-end' : 'center')), alignItems: (block.valign === 'top' ? 'flex-start' : (block.valign === 'bottom' ? 'flex-end' : 'center')) }">
                                <template x-if="block.link_url">
                                    <a href="javascript:void(0)" style="display:inline-block; max-width: 100%;" :style="{ textAlign: block.align || 'center' }">
                                        <img :src="block.src || '/admin/images/placeholder.svg'" :alt="block.alt" :style="{ maxWidth: block.max_width || '100%', width: block.width || 'auto', objectFit: block.fit || 'contain', borderRadius: (block.border_radius || 8) + 'px' }" style="height:auto;display:inline-block">
                                    </a>
                                </template>
                                <template x-if="!block.link_url">
                                    <img :src="block.src || '/admin/images/placeholder.svg'" :alt="block.alt" :style="{ maxWidth: block.max_width || '100%', width: block.width || 'auto', objectFit: block.fit || 'contain', borderRadius: (block.border_radius || 8) + 'px' }" style="height:auto;display:inline-block">
...
function popupEditor(initial) {
    ...
    initial.settings = {
        ...
        trigger: initial.settings.trigger || 'page_load',
        clickSelector: initial.settings.clickSelector || '',
        scrollPercent: initial.settings.scrollPercent || 50,
    };
    initial.design = {
    ...
    // Page targeting defaults
    if (!initial.page_targeting_mode) initial.page_targeting_mode = 'all';
    if (!initial.page_paths) initial.page_paths = [];
    ...
    return {
        ...
        addBlock(type) {
            const blocks = {
                ...
                image: { _key: 'blk_' + keyCounter++, type: 'image', src: '', alt: '', link_url: '', width: '100%', max_width: '100%', border_radius: 8, align: 'center', valign: 'middle', fit: 'contain' },
        ...
        addPath() {
            const p = this.normalizePath(this.data.newPath || '');
            if (!p) return;
            if (!this.data.page_paths) this.data.page_paths = [];
            if (!this.data.page_paths.includes(p)) {
                this.data.page_paths.push(p);
            }
            this.data.newPath = '';
        },
        ...
        normalizePath(value) {
            ...
            if (!path || path === '/' || path === 'home' || path === '/home') return '/';
            try {
        ...
        removePath(idx) {
            this.data.page_paths.splice(idx, 1);
        },
